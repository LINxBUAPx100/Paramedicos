...
import { relieveDeImagen } from './relieveFoto.js'
...
// Las fotos viven en public/imagenes/. `BASE_URL` importa: la app se publica
// con `base: './'` en un subdirectorio de GitHub Pages, así que una ruta
// absoluta apuntaría a la raíz del dominio y no al repositorio.
const BASE_IMAGENES = `${import.meta.env.BASE_URL || './'}imagenes/`

// Una pieza fotográfica: la foto del objeto real, extruida por su propia
// silueta para que tenga volumen, plantada en la bandeja con el tamaño y la
// orientación que declara piezas.js.
//
// Se prefiere a la forma procedural porque una gasa, un apósito y un sobre de
// algodón son la misma caja en primitivas, y la foto los distingue de un golpe
// de vista. Mientras la foto viaja se muestra la forma procedural; si no llega,
// se queda.
function recorteFotografico(THREE, articulo, ctx, respaldo) {
  const visual = articulo.visual
  const grupo = new THREE.Group()

  // Sombra de contacto: sin ella la pieza parece flotar sobre la bandeja.
  const sombra = new THREE.Mesh(
    new THREE.CircleGeometry(0.4, 24),
    new THREE.MeshBasicMaterial({ color: '#020617', transparent: true, opacity: 0.24, depthWrite: false }),
  )
  sombra.rotation.x = -Math.PI / 2
  sombra.position.y = 0.008
  grupo.add(sombra)

  // MeshBasicMaterial y no Standard: la foto ya trae su propia luz, y volver a
  // sombrearla la ensucia. `alphaTest` recorta el fondo transparente sin dejar
  // el halo que produce la mezcla por orden de dibujado.
  const materialFoto = new THREE.MeshBasicMaterial({
    transparent: true,
    alphaTest: 0.2,
    side: THREE.DoubleSide,
    toneMapped: false,
  })
  materialFoto.userData.esFoto = true

  // El canto del sólido extruido. La foto no dice de qué color es el interior
  // del objeto, así que se usa el color declarado del artículo, oscurecido.
  const materialCanto = new THREE.MeshStandardMaterial({
    color: colorSeguro(THREE, visual?.color).multiplyScalar(0.62),
    roughness: 0.72,
    metalness: 0.04,
  })
  materialCanto.userData.esFoto = true
  ...
  ctx.cargador.load(
    `${BASE_IMAGENES}${visual.imagen}`,
    (textura) => {
      textura.colorSpace = THREE.SRGBColorSpace
      ctx.texturas.push(textura)
      materialFoto.map = textura
      materialFoto.needsUpdate = true

      const ancho = textura.image?.width || 1
      const alto = textura.image?.height || 1
      // El ALTO manda y el ancho sale de la proporción real de la foto: el
      // layout ya plantó el objeto midiendo alto 1, así que cambiar solo el
      // ancho no lo despega de la bandeja.
      const proporcion = Math.min(2.4, Math.max(0.3, ancho / alto))

      let malla = null
      if (visual.grosorRelativo > 0) {
        // Grosor de verdad: se traza el contorno del canal alfa y se extruye,
        // así que el canto sigue la silueta del objeto en vez de ser un
        // rectángulo. Si la foto no da una silueta usable (sin transparencia,
        // o de otro origen) devuelve null y se cae a la lámina.
        // El relieve trae presupuestos y devuelve null cuando la foto no da una
        // silueta razonable. El try es la última red: pase lo que pase con una
        // imagen concreta, la pieza se dibuja como lámina y la escena sigue.
        try {
          const geometria = relieveDeImagen(THREE, textura.image, { grosor: visual.grosorRelativo })
          if (geometria) malla = new THREE.Mesh(geometria, [materialFoto, materialCanto])
        } catch {
          malla = null
        }
      }
      if (!malla) {
        const lamina = new THREE.PlaneGeometry(proporcion, 1)
        malla = new THREE.Mesh(lamina, materialFoto)
      }
      malla.castShadow = true
      malla.receiveShadow = true
      malla.userData.itemId = articulo.id
      pieza.add(malla)

      // 'acostada': tendida en la bandeja, como al abrir el botiquín y verla
      // desde arriba. El grosor pasa a ser su altura, que es justo lo que se ve
      // de un estetoscopio o unas tijeras dentro de su compartimento.
      if (visual.orientacion === 'acostada') {
        pieza.rotation.set(-Math.PI / 2, 0, (visual.giro || 0) * (Math.PI / 180))
      } else {
        pieza.position.y = 0.5
        if (visual.orientacion === 'fija') pieza.rotation.y = (visual.giro || 0) * (Math.PI / 180)
        else ctx.carteles.push(pieza)
      }

      // La sombra se ajusta a lo que ocupa la pieza en el suelo de la bandeja.
      const huella = visual.orientacion === 'acostada' ? Math.max(proporcion, 1) : proporcion
      sombra.scale.setScalar(Math.max(0.45, huella * 0.62))

      pieza.visible = true
      if (respaldo) respaldo.visible = false
      // El objeto ya no mide lo mismo que el placeholder: hay que volver a
      // apoyarlo, o queda hundido en la bandeja o flotando sobre ella.
      ctx.reasentar?.(articulo.id)
      ctx.pickables?.push(malla)
    },
    undefined,
    () => { pieza.visible = false },
  )
...
function crearObjeto(THREE, articulo, ctx) {
  const grupo = new THREE.Group()
  ...
  if (ctx && articulo.visual?.imagen) {
    const contenedor = new THREE.Group()
    contenedor.add(grupo)
    contenedor.add(recorteFotografico(THREE, articulo, ctx, grupo))
    return contenedor
  }
  ...
  raiz.traverse((objeto) => {
    ...
    for (const mat of materiales) {
      ...
      if (bloqueado || silueta) {
        if (mat.userData.esFoto) {
          mat.color?.set(silueta ? '#020617' : '#1e293b')
          mat.transparent = true
          mat.opacity = silueta ? 0.5 : 0.62
        } else {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const texturas = []
        // Los carteles se reorientan cada fotograma; el cargador es uno solo
        // para que las fotos compartan caché de red.
        const carteles = []
        const roots = new Map()
        const pickables = []
        const ctxImagenes = {
          ...
          // La foto llega despues del reparto: la pieza que trae ya no mide lo
          // que medía el respaldo, así que hay que volver a apoyarla o queda
          // hundida en la bandeja o flotando sobre ella.
          reasentar: (id) => {
            const raiz = roots.get(id)
            if (!raiz) return
            raiz.position.y = 0
            raiz.updateMatrixWorld(true)
            const caja = new THREE.Box3().setFromObject(raiz)
            raiz.position.y = 0.08 - caja.min.y
        ...
        for (const comp of COMPARTIMENTOS_BOTIQUIN) {
          ...
          items.forEach((item, indice) => {
            const raiz = crearObjeto(THREE, item, ctxImagenes)
            raiz.userData.itemId = item.id
            raiz.userData.phase = indice * 0.73 + COMPARTIMENTOS_BOTIQUIN.indexOf(comp)
            // `factor` viene de piezas.js: es el tamaño real del objeto medido
            // contra la referencia de 12 cm, ya con su tope aplicado.
            raiz.userData.baseScale = escalaBase * (item.visual?.escala || 1) * (item.visual?.factor || 1)
        ...
        const posicionCartel = new THREE.Vector3()
        ...
        const animar = (ahora) => {
          ...
          // Solo el eje Y: copiar la orientación completa de la cámara
          // inclinaría los recortes y los dejaría flotando de canto.
          if (carteles.length) {
            for (const cartel of carteles) {
              cartel.getWorldPosition(posicionCartel)
              cartel.rotation.y = Math.atan2(
                camara.position.x - posicionCartel.x,
                camara.position.z - posicionCartel.z,
              )
            }
          }
          if (!reduced) {
