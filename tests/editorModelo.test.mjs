...
import {
  ...
  crearModulo, crearUnidad, crearTema, actualizarNodo,
...
import { ensamblarModulos } from '../src/lib/contenidoApi.js'

// Estructura de juego: 2 módulos, la primera con 2 unidades y 3 temas.
function base() {
  return [
    {
      ...
      unidades: [
        {
          id: 'm1', titulo: 'Unidad 1', estado: 'publicado',
          temas: [
            { id: 't1', titulo: 'Tema 1', estado: 'publicado' },
            { id: 't2', titulo: 'Tema 2', estado: 'borrador' },
          ],
        },
        { id: 'm2', titulo: 'Unidad 2', estado: 'publicado', temas: [{ id: 't3', titulo: 'Tema 3', estado: 'publicado' }] },
      ],
    },
    { id: 'f2', titulo: 'Fase 2', estado: 'publicado', unidades: [] },
  ]
}

// ---------- creación y jerarquía ----------

test('crear: modulo, unidad y tema nacen en borrador y con ids únicos', () => {
  let e = base()
  const rf = crearModulo(e, { titulo: 'Módulo nueva' })
  assert.equal(rf.nodo.estado, 'borrador')
  assert.equal(rf.nodo.id, 'modulo-nueva')
  const rm = crearUnidad(rf.estructura, { moduloId: 'modulo-nueva', titulo: 'Unidad nuevo' })
  assert.equal(rm.nodo.estado, 'borrador')
  const rt = crearTema(rm.estructura, { moduloId: 'modulo-nueva', unidadId: 'unidad-nuevo', titulo: 'Tema 1' })
...
test('crear: id de tema único a nivel CURSO (no solo por unidad)', () => {
  let e = base()
  const r1 = crearTema(e, { moduloId: 'f1', unidadId: 'm1', titulo: 'RCP' })
  const r2 = crearTema(r1.estructura, { moduloId: 'f1', unidadId: 'm2', titulo: 'RCP' })
...
test('crear: título vacío o demasiado largo se rechaza', () => {
  assert.throws(() => crearModulo(base(), { titulo: '   ' }), /título/i)
  assert.throws(() => crearTema(base(), { moduloId: 'f1', unidadId: 'm1', titulo: 'x'.repeat(LIMITE_TITULO + 1) }), /largo/i)
  assert.equal(validarTitulo('Correcto'), null)
})

test('crear: padre inválido o archivado se rechaza', () => {
  assert.throws(() => crearUnidad(base(), { moduloId: 'no-existe', titulo: 'X' }), /No existe el módulo/)
  assert.throws(() => crearTema(base(), { moduloId: 'f1', unidadId: 'no-existe', titulo: 'X' }), /No existe la unidad/)
  const { estructura } = archivarNodo(base(), { moduloId: 'f1' })
  assert.throws(() => crearUnidad(estructura, { moduloId: 'f1', titulo: 'X' }), /archivad[ao]/)
})

// ---------- edición ----------

test('actualizar: solo campos permitidos por tipo', () => {
  const r = actualizarNodo(base(), { moduloId: 'f1' }, { titulo: 'Módulo renombrada', descripcion: 'Desc' })
  assert.equal(r.nodo.titulo, 'Módulo renombrada')
  assert.throws(() => actualizarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }, { resumen: 'x' }), /no se puede editar/)
  assert.throws(() => actualizarNodo(base(), { moduloId: 'f1' }, { estado: 'publicado' }), /no se puede editar/)
})

// ---------- orden ----------

test('reordenar: arriba/abajo/inicio/fin y límites', () => {
  let r = reordenarNodo(base(), { moduloId: 'f2' }, 'arriba')
  assert.deepEqual(r.estructura.map((f) => f.id), ['f2', 'f1'])
  r = reordenarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }, 'fin')
  assert.deepEqual(r.estructura[0].unidades[0].temas.map((t) => t.id), ['t2', 't1'])
  r = reordenarNodo(base(), { moduloId: 'f1', unidadId: 'm2' }, 'inicio')
  assert.deepEqual(r.estructura[0].unidades.map((m) => m.id), ['m2', 'm1'])
  assert.throws(() => reordenarNodo(base(), { moduloId: 'f1' }, 'arriba'), /extremo/)
  assert.throws(() => reordenarNodo(base(), { moduloId: 'f1' }, 'diagonal'), /inválido/)
})

// ---------- mover entre padres ----------

test('mover: tema a otra unidad y unidad a otro módulo del mismo curso', () => {
  let r = moverNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }, { moduloId: 'f1', unidadId: 'm2' })
  assert.deepEqual(r.estructura[0].unidades[0].temas.map((t) => t.id), ['t2'])
  assert.deepEqual(r.estructura[0].unidades[1].temas.map((t) => t.id), ['t3', 't1'])
  r = moverNodo(base(), { moduloId: 'f1', unidadId: 'm2' }, { moduloId: 'f2' })
  assert.deepEqual(r.estructura[0].unidades.map((m) => m.id), ['m1'])
  assert.deepEqual(r.estructura[1].unidades.map((m) => m.id), ['m2'])
})

test('mover: destino inválido, archivado o el mismo padre se rechaza', () => {
  assert.throws(() => moverNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }, { moduloId: 'f9', unidadId: 'm9' }), /No existe/)
  assert.throws(() => moverNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }, { moduloId: 'f1', unidadId: 'm1' }), /ya está/)
  const { estructura } = archivarNodo(base(), { moduloId: 'f2' })
  assert.throws(() => moverNodo(estructura, { moduloId: 'f1', unidadId: 'm1' }, { moduloId: 'f2' }), /archivad[ao]/)
  assert.throws(() => moverNodo(base(), { moduloId: 'f1' }, { moduloId: 'f2' }), /no se mueven/)
})

// ---------- duplicación ----------

test('duplicar tema: copia independiente en borrador con id nuevo y mapeo', () => {
  const r = duplicarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' })
  const temas = r.estructura[0].unidades[0].temas
...
test('duplicar modulo: nueva rama completa con temas re-identificados', () => {
  const r = duplicarNodo(base(), { moduloId: 'f1' })
...
test('archivar/restaurar: reversible y sin borrar nada; cuenta descendientes activos', () => {
  assert.equal(contarDescendientesActivos(base(), { moduloId: 'f1' }), 5) // 2 unidades + 3 temas
  const a = archivarNodo(base(), { moduloId: 'f1', unidadId: 'm1' })
  assert.equal(a.estructura[0].unidades[0].estado, 'archivado')
  // Los hijos conservan su estado propio (política documentada).
  assert.equal(a.estructura[0].unidades[0].temas[0].estado, 'publicado')
  const rst = restaurarNodo(a.estructura, { moduloId: 'f1', unidadId: 'm1' })
  assert.equal(rst.nodo.estado, 'borrador')
  assert.throws(() => restaurarNodo(base(), { moduloId: 'f1' }), /no está archivado/)
})

test('archivar: la rama archivada desaparece de lo que recibe el alumno', () => {
  const { estructura } = archivarNodo(base(), { moduloId: 'f1', unidadId: 'm1' })
  const docs = new Map([
  ...
  assert.deepEqual(modulos[0].temas.map((t) => t.id), ['t3'])
})

// ---------- publicación ----------

test('publicar: valida título y bloquea ancestros archivados', () => {
  const ok = publicarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't2' })
  assert.equal(ok.nodo.estado, 'publicado')
  // Con la unidad archivada, su tema no se publica.
  const a = archivarNodo(base(), { moduloId: 'f1', unidadId: 'm1' })
  assert.throws(
    () => publicarNodo(a.estructura, { moduloId: 'f1', unidadId: 'm1', temaId: 't2' }),
    /unidad .* archivada/i
  )
  // Con el módulo archivado, ni unidades ni temas.
  const af = archivarNodo(base(), { moduloId: 'f1' })
  assert.throws(() => publicarNodo(af.estructura, { moduloId: 'f1', unidadId: 'm1' }), /módulo .* archivado/i)
  // Con el curso archivado, nada.
  assert.throws(() => publicarNodo(base(), { moduloId: 'f1' }, { cursoEstado: 'archivado' }), /curso está archivado/)
  // Un nodo archivado primero se restaura.
  const at = archivarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' })
  assert.throws(() => publicarNodo(at.estructura, { moduloId: 'f1', unidadId: 'm1', temaId: 't1' }), /restáuralo/)
})

test('despublicar: publicado → borrador; el resto se rechaza', () => {
  const r = despublicarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't1' })
  assert.equal(r.nodo.estado, 'borrador')
  assert.throws(() => despublicarNodo(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't2' }), /no está publicado/)
})
...
test('tipoDeRef y localizar: refs bien tipadas y errores claros', () => {
  assert.equal(tipoDeRef({ moduloId: 'f1' }), 'modulo')
  assert.equal(tipoDeRef({ moduloId: 'f1', unidadId: 'm1' }), 'unidad')
  assert.equal(tipoDeRef({ moduloId: 'f1', unidadId: 'm1', temaId: 't1' }), 'tema')
  assert.equal(tipoDeRef(null), null)
  assert.equal(localizar(base(), { moduloId: 'f1', unidadId: 'm1', temaId: 't2' }).tema.titulo, 'Tema 2')
})
...
test('validarEstructura: detecta duplicados y estados inválidos', () => {
  ...
  const mal = base()
  mal[0].unidades[0].temas[0].estado = 'visible'
